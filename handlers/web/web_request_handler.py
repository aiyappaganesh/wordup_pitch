...
class WebRequestHandler(RequestHandler):
    ...
    def get_rendered_html(self, template_name, template_values = None):
        if not template_name:
            ...
            template = 'desktop/' + path + template_name
            ua = self.request.headers['User-Agent']
            b = reg_b.search(ua)
            v = reg_v.search(ua[0:4])
            android = reg_android.search(ua)
            logging.info('*** UA: '+str(ua)+' , *** Is Android: '+str(True if android else False))
            if b or v:
                logging.info('*** mobile ***')
                try:
                    with open('templates/mobile/'+path+template_name) as template_file:
                        template = 'mobile/' + path + template_name
                except IOError as e:
                    pass
                if android:
                    template_values['android'] = True
                else:
                    template_values['android'] = False
        else:
            template = template_name
        return loader.render_to_string(template, template_values)